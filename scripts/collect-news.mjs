...
export function mergeNews(results, old, now = Date.now()) {
  const successful = results.flatMap(result => result.status === 'fulfilled' ? result.value.items : []);
  const failedIds = new Set(results.flatMap((result, index) => result.status === 'rejected' ? [FEEDS[index].id] : []));
  const retained = (old?.news || []).filter(item => failedIds.has(item.sourceId)
    && Number.isFinite(Date.parse(item.publishedAt))
    && now - Date.parse(item.publishedAt) <= 7 * 86400000);
  return [...new Map([...retained, ...successful].map(item => [item.id, item])).values()]
...
export async function collect() {
  ...
  if (results.every(result => result.status === 'rejected')) throw new Error('All official news feeds failed; prior snapshot retained');
  const news = mergeNews(results, old);
  let events = [];
  try { events = parseBlsCalendar(await fetchText('https://www.bls.gov/schedule/news_release/current_year.asp')); }
  catch (error) { sources.push({ id: 'bls-calendar', status: 'error', message: String(error.message).slice(0,100) }); events = (old?.events ?? []).filter(event => Date.parse(event.occursAt) >= Date.now()); }
