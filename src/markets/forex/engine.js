...
function modelPair(definition, history, session) {
  ...
  return { symbol: definition.id, id: definition.id, baseCurrency: definition.base, quoteCurrency: definition.quote, base: definition.base, quote: definition.quote, label: definition.label, price, current: price, changePct, change: changePct, high, high20: high, low, low20: low, volume: null, activityProxy: session.activity, spread: null, session: session.id, timestamp: history.updatedAt, candles: series.map((point, index) => ({ time: point.date, open: series[index - 1]?.value ?? point.value, high: Math.max(series[index - 1]?.value ?? point.value, point.value), low: Math.min(series[index - 1]?.value ?? point.value, point.value), close: point.value })), series, momentum, trend, position: clamp(position) };
}
