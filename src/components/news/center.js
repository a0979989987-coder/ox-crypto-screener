(() => {
  ...
  const state = { snapshot: null, lastError: null, pending: null, tab: 'overview', previous: null, request: 0 };
  ...
  const fmt = value => new Intl.DateTimeFormat('zh-TW', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(value));
  ...
  function filtered(kind, market, onlyFollowing = false) {
    ...
    items = items.filter(item => (!market || item.markets?.includes(market)) && !hidden.has(item.id) && !sources.has(item.sourceId));
    if (onlyFollowing) items = items.filter(item => (prefs.followedSources || []).includes(item.sourceId));
    if (kind === 'news' && prefs.majorOnly !== false) items = items.filter(item => (score(item) ?? 0) >= 3);
  ...
  function card(item) {
    ...
    actions.append(hide);
    const follow = el('button', '', (store().followedSources || []).includes(item.sourceId) ? '已追蹤來源' : '追蹤來源'); follow.type = 'button';
  ...
  function controls(surface, isAll) {
    ...
    const prefs = store();
    const major = el('button', 'ox-news-filter', prefs.majorOnly === false ? '全部新聞' : '重大優先'); major.type = 'button'; major.setAttribute('aria-pressed', String(prefs.majorOnly !== false));
    major.addEventListener('click', () => { save({ majorOnly: prefs.majorOnly === false }); render(); }); row.append(major);
    ...
    const reload = el('button', 'ox-news-filter', '更新'); reload.type = 'button'; reload.addEventListener('click', () => refresh(true)); row.append(reload);
    wrap.append(row);
    const panel = el('div', 'ox-news-options'); panel.hidden = true;
    options.addEventListener('click', () => { panel.hidden = !panel.hidden; options.setAttribute('aria-expanded', String(!panel.hidden)); });
  ...
  function renderSurface(surface, market) {
    ...
    else if (snapshot) status.textContent = `資料快照：${fmt(snapshot.generatedAt)} · 定時彙整，並非逐秒即時新聞`;
  ...
  function render() {
    ...
    const local = document.querySelector('[data-news-surface="market"]'); if (local) renderSurface(local, market);
    const all = document.querySelector('[data-news-surface="all"]'); if (all) renderSurface(all, null);
  ...
  function open({ historyEntry = true } = {}) {
    if (document.body.dataset.newsMode !== '1') state.previous = { view: document.querySelector('.app-view.active')?.dataset.appView || 'home', market: currentMarket(), scroll: window.scrollY };
    document.body.dataset.newsMode = '1'; state.tab = 'overview';
    window.switchAppView?.('news');
    if (historyEntry) history.pushState({ oxNews: true }, '', '#news');
    refresh();
  }
  ...
  document.addEventListener('click', event => {
    ...
    if (event.target.closest('#ox-open-settings')) { event.preventDefault(); document.getElementById('ox-control-close')?.click(); close(); window.switchAppView?.('settings'); }
    const tab = event.target.closest('[data-news-tab]'); if (tab) { state.tab = tab.dataset.newsTab; render(); window.scrollTo(0,0); }
  });
  document.addEventListener('ox:marketchange', () => { if (document.body.dataset.newsMode === '1') close(); render(); });
  document.addEventListener('ox:viewchange', event => { if (event.detail?.to === 'data' || event.detail?.to === 'news') refresh(); });
  window.addEventListener('popstate', event => { if (event.state?.oxNews) open({ historyEntry: false }); else if (document.body.dataset.newsMode === '1') close(); });
